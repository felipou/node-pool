...
exports.Pool = function (factory) {
  ...
  /**
   * Request the resource be destroyed. 
   *
   * Removes the object from the available list and calls the factory's destroy
   * handler on it.
   * 
   * @param {Object} obj
   ...
   *
   *  - If there are resources available, shift the first one out (LIFO),
   *    and call its callback.
   *  - If there are no resources available, try to create one if it won't exceed
   *    the maximum number of resources.
   *  - If creating a new resource would exceed the maximum, add the client to
   *    the wait list.
   */
  function dispense() {
    var obj = null,
  ...
  /**
   * Request a resource. The callback will be called,
   * when a resource is available, passing the resource to it.
   *
   ...
   *
   * @param {Object} obj
   *   The acquired resource to be put back to the pool.
   */
  ...
  /**
   * Return the resource to the pool to be destroyed.
   *
   * 
   * This could be called within an acquire() block as an alternative to release(). 
   *
   * @param {Object} obj
  ...
  me.destroy = function(obj) {
    // check to see if this object has already been released (i.e., is back in the pool of availableObjects)
    if (availableObjects.some(function(objWithTimeout) { return (objWithTimeout.obj === obj); })) {
      log("destroy called for an already released resource: " + (new Error().stack), 'error');
      return;
    } 
    count -= 1;
    factory.destroy(obj);
  ...
  /**
   * Disallow any new resource requests and let the request backlog dissapate.
   *
   * @param {Function} callback
